...
app.controller('goodsController' ,function($scope,$controller,$location  ,
    ...
    $scope.updateDele=function(num){
        goodsService.updateDele($scope.selectIds,num).success(
            function(response){
                if(response.success){
                    $scope.reloadList();//刷新列表
                    $scope.selectIds=[];//清空 ID 集合
  ...
  //查询二级商品分类列表
	...
	$scope.$watch('entity.goods.category3Id',function(newValue,oldValue){
		...
		itemCatService.findOne(newValue).success(
				function(response){
					$scope.entity.goods.typeTemplatedId=response.typeId;//更新模板			
				}
		);
		
	});
	//更新商品
	$scope.$watch('entity.goods.typeTemplatedId',function(newValue,oldValue){
